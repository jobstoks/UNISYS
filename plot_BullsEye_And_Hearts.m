...
if exist('Vq_all','var')
    ...
    if length(Vq_all)==1
        ...
        for i=1:length(Vq_all)
            allvals=[allvals; Vq_all{i}];
            allvals_separated{i}=Vq_all{i};
        end
    end
else
    for i=1:length(bullseye)
...
if nargin==4 && exist('dev_opts','var')
    if isfield(dev_opts,'plot')
        ...
        if isfield(dev_opts.plot,'colorbar_bullseyes')
            showbar_bullseye=dev_opts.plot.colorbar_bullseyes;
        else
            showbar_bullseye=default_showclrbar;
        end
        if isfield(dev_opts.plot,'colorbar_hearts')
            showbar_heart=dev_opts.plot.colorbar_hearts;
        else
...
for numplot=1:maxlength
    if show_bullseyes==1
        if numplot<=length(bullseye)
            if doPlot
                ax(id_plot)=subtightplot(numrows,numplotsperrow,id_plot);
            end
            %If Vq (the values on the grid) are defined as an input: work from
            %grid, and translate back to the heart.
            if exist('Vq_all','var')
            ...
            if doPlot
                ...
                p(id_plot) = patch('Faces',tri,'Vertices',bullseye(numplot).grid);
                ...
                elseif showbar_bullseye==1
                    show_here=1;
                elseif showbar_bullseye==2
                    if id_plot==numrows*numplotsperrow-1 && show_hearts==1
                        show_here=1;
...
function clrbar_vals= set_clrs(numplot,lims,flipud_var,steps,map,map_spec,symmetrical,showbar)
